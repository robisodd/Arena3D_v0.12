...
#ifdef PBL_COLOR
  GColor black_white_palette[2]={{.argb=0b11000000},{.argb=0b11111111}};  // Used when converting GBitmapFormat1Bit to GBitmapFormat1BitPalette
#endif
    ...
    if(texture[i].bmp==NULL) {
      APP_LOG(APP_LOG_LEVEL_DEBUG, "Failed to load texture: %d", i);
        ...
        switch (gbitmap_get_format(texture[i].bmp)) {
          ...
          default:                       texture[i].format=3; texture[i].bytes_per_row=0; texture[i].pixels_per_byte = 8; texture[i].colormax= 0; break; //pixels_per_byte is set for 8 to shift mapxmapy&63 to 0
